  ...
  return (
    ...
    >
      <div className="my-card-image-container relative ">
        ...
        <img src={img} className="object-contain rounded-sm" alt={title} />
        <p className="my-card-description backdrop-blur-sm ">{descr}</p>
      </div>

      <Link to={`/details/${id}`}>
        <div className="my-card-title text-white text-lg font-semibold transition-all duration-100 hover:bg-orange-600">{title}</div>
      </Link>
    </div>
  );
};
