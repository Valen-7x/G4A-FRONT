...
import { increaseQuantity, decreaseQuantity, updateQuantity, removeFromCart } from "../redux/actions/cart.js";
...
export default function CartView() {
  ...
  // ------ TOTAL ------
  const totalCartPrice = cartItems.reduce((total, item) => total + item.game.game.price * item.quantity, 0).toFixed(2);
  ...
  return (
    ...
    md:bottom-0"
                ...
                <div>
                  ...
                  >
                    <div>
                      <p className="font-bold text-[30px] text-left">{item.game.game.name}</p>
                      <p className="text-left text-[#fffc]">{item.game.game.short_description}</p>
                    </div>
                    <div
                      className="
                    md:flex md:flex-row-reverse md:h-[3rem] md:justify-between 
                    md:items-center md:w-[100%]
                    md:mt-5 md:mb-1"
                    >
                      <p className="font-semibold text-[20px]">${(item.game.game.price * item.quantity).toFixed(2)}</p>
                      {/* ------ QUANTITY ------*/}
                      <div className="flex shadow-[2px_2px_5px_1px_rgba(0,0,0,0.25)]">
                        <button className="px-2 font-bold bg-[#E58D27]" onClick={() => handleIncreaseQuantity(item.id, 3)}>
                          +
                        </button>

                        <p className="w-[3ch] font-bold px-1 bg-gray-600">{item.quantity}</p>
                        <button
                          className={`px-2 font-bold bg-[#E58D27] ${item.quantity === 1 ? "cursor-not-allowed" : ""}`}
      ...
      <Market className="w-[100%]" total={totalCartPrice} />
    </div>
  );
}
